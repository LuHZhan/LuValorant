...
namespace GMP
{
	...
	namespace Hub
	{
		...
		struct TMessageTraits
		{
			...
			static FGMPMessageSig MakeCallback(FMessageHub *InMsgHub, F &&Func, std::true_type)
			{
				...
				return [Func{std::move(Func)}, InMsgHub](FMessageBody &Body)
				{ Hub::InvokeWithSingleShotInfo(InMsgHub, static_cast<const AttachedFunctorType &>(Func), Body, (ReducedTuple *)nullptr, (SeqIndex *)nullptr); };
			}

			template <typename F>
			static FGMPMessageSig MakeCallback(FMessageHub *, F &&Func, std::false_type)
			{
				return [Func{std::move(Func)}](FMessageBody &Body)
				{ Hub::Invoke<Tuple>(static_cast<const AttachedFunctorType &>(Func), Body); };
			}
		};
		...
		struct TListenArgumentsTraits
		{
			...
			static decltype(auto) MakeCallback(FMessageHub *InMsgHub, T *Listener, R (F::*Op)(TArgs...))
			{
				...
				auto Func = [=](ForwardParam<TArgs>... Args)
				{ return (Listener->*Op)(static_cast<TArgs>(Args)...); };
		...
		// TypeTraits::TIsCallable<LastType>::value：检查 LastType 是否是一个可调用类型（如函数、lambda、函数对象等）。
	...
	public:
		 ...
		 * 发送消息的通用接口函数
		 *
		 * @param MessageKey - 消息的唯一标识符
		 * @param Args - 可变参数包，用于传递消息的参数
		 * @return uint64 - 返回消息的序列号，如果发送失败返回0
		 *
		 * @note 这是一个便捷的包装函数，内部会调用 SendObjectMessage 并将 FSigSource 设置为 nullptr
		 * @note 支持自动类型推导和完美转发
		 *
		 * 使用示例:
		 * @code
		 * hub.SendMessage("MyMessage", 42, "Hello", playerPtr);
		 * @endcode
		 */
		template <typename... TArgs>
		 ...
		 *
		 * @param MessageKey - 消息的唯一标识符
		 * @param InSigSrc - 消息源对象，用于追踪消息的发送者
		 * @param Args - 可变参数包，用于传递消息的参数
		 * @return uint64 - 返回消息的序列号，如果发送失败返回0
		 */
		template <typename... TArgs>
		uint64 SendObjectMessage(const FMSGKEYFind &MessageKey, FSigSource InSigSrc, TArgs &&...Args)
		{
			...
			if (Ptr)
			{
				...
				// 根据是否需要响应，调用不同的消息发送实现
				// 如果最后一个参数是回调函数，则创建单次响应处理
				return SendObjectMessageImpl(Ptr, MessageKey, InSigSrc, Arr,
											 SendTraits::MakeSingleShot(MessageKey, &TupRef));
			}
			return 0;
		}
	...
	public: // for script binding
		...
		{
			auto Func = [=](FMessageBody &Body)
			{ (Listener->*MemFunc)(Body); };
	...
	namespace Hub
	{
		...
		{
			return FResponeSig([](FMessageBody &Body)
							   { GMP_ERROR(TEXT("ResponeMessage Mismatch")); }, SingleShotId, 0u);
		}
#endif
		template <typename F>
		FResponeSig DefaultLessTraits::MakeSingleShotImpl(const FName &SingleShotId, F &&OnRsp)
		{
			...
			return FResponeSig([OnRsp{std::forward<F>(OnRsp)}](FMessageBody &Body)
							   { Hub::Invoke<typename SingleshotTraits::Tuple>(OnRsp, Body); }, SingleShotId, FMessageBody::GetNextSequenceID());
		}
	} // namespace Hub
} // namespace GMP
